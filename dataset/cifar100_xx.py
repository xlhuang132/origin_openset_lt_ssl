...
def get_cifar100_test_dataset(root,transform_val=None):
    test_dataset = CIFAR100_labeled(root, train=False, transform=transform_val, download=True)
    return test_dataset

def get_cifar100_for_ood_detection(root, out_dataset, start_label=0, 
                  transform_val=None,
    ...
    # 所有的训练数据连接在一起
    train_dataset = CIFAR100_cocnat(root, train_labeled_idxs, train_unlabeled_idxs, start_label, OOD_path=f'./data/{out_dataset}', train=True,ood_train=False, transform=transform_val)
...
def get_cifar100(root, out_dataset, start_label=0,ood_ratio=0, 
    ...
    train_labeled_dataset = CIFAR100_labeled(root, train_labeled_idxs, train=True, transform=transform_train,num_per_cls_list=l_num_per_cls_list)
    if cfg.ALGORITHM.NAME in["Ours","OODMix","CReST"]:
        train_unlabeled_dataset = CIFAR100_unlabeled(root, train_unlabeled_idxs, OOD_path=f'./data/{out_dataset}', ood_num=ood_num,train=True, transform=TransformTwice(transform_train),return_index=True)
    else:
        train_unlabeled_dataset = CIFAR100_unlabeled(root, train_unlabeled_idxs, OOD_path=f'./data/{out_dataset}', ood_num=ood_num,train=True, transform=TransformTwice(transform_train))
    
    if cfg.ALGORITHM.PRE_TRAIN.SimCLR.ENABLE:
        transform=build_simclr_transform(cfg)
        pre_train_dataset = CIFAR100_cocnat_pair(root, train_labeled_idxs, train_unlabeled_idxs,
                                                 start_label, OOD_path=f'./data/{out_dataset}', 
                                                 transform=transform,
                                                 train=True,ood_num=ood_num)
 # 所有的训练数据连接在一起
  
    train_dataset = CIFAR100_cocnat(root, train_labeled_idxs, train_unlabeled_idxs, start_label, OOD_path=f'./data/{out_dataset}', train=True,ood_num=ood_num, transform=transform_train)
...
class CIFAR100_cocnat(torchvision.datasets.CIFAR100):
    ...
    def __init__(self, root, labeled_indexs, unlabeled_indexs, start_label, OOD_path,ood_num=None, train=True,
                 ...
                 download=False):
        super(CIFAR100_cocnat, self).__init__(root, train=train,
                 transform=transform, target_transform=target_transform,
                 download=download)
...
class CIFAR100_cocnat_pair(torchvision.datasets.CIFAR100):

    def __init__(self, root, labeled_indexs, unlabeled_indexs, start_label, OOD_path,ood_num=None, train=True,
                 transform=None, target_transform=None,ood_train=True,
                 download=False):
        super(CIFAR100_cocnat_pair, self).__init__(root, train=train,
                 transform=transform, target_transform=target_transform,
                 download=download)
